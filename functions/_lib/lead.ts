...
export async function handleLead(
  ...
  if (
    ...
    !origin.endsWith(".comfortline-codex.pages.dev")
  )
  ...
  if (!name || phone.replace(/\D/g, "").length < 9)
    return response({ error: "Invalid contact details" }, 400);

  const payload = Object.fromEntries(
    Object.entries(raw).map(([key, value]) => [
      key.slice(0, 50),
      clean(value, key === "comment" || key === "message" ? 1500 : 300),
    ]),
  );
  const title =
    kind === "booking"
      ? "New ComfortLine booking"
      : "New ComfortLine contact request";
  const text = [
    title,
    ...
    const webhook = await fetch(env.LEAD_WEBHOOK_URL, {
      ...
      headers: {
        ...
        ...(env.LEAD_WEBHOOK_SECRET
          ? { Authorization: `Bearer ${env.LEAD_WEBHOOK_SECRET}` }
          : {}),
      },
      body: JSON.stringify({
        type: kind,
        source: "comfortline.by",
