export const SITE_URL = "https://comfortline.by";

export const DEFAULT_OG_IMAGE = `${SITE_URL}/opengraph.jpg`;

export const BUSINESS = {
  name: "ComfortLine",
  legalName: "ComfortLine Transfer",
  phone: "+375447620649",
  phoneDisplay: "+375 (44) 762-06-49",
  email: "info@comfortline.by",
  city: "Minsk",
  country: "BY",
  url: SITE_URL,
  logo: `${SITE_URL}/favicon.svg`,
  image: `${SITE_URL}/opengraph.jpg`,
  sameAs: [
    "https://t.me/+375447620649",
    "https://wa.me/375447620649",
    "https://www.instagram.com/transfer_comfortline/",
    "https://m.me/103816619260365",
  ],
  serviceArea: [
    "Minsk",
    "Belarus",
    "Lithuania",
    "Poland",
    "Latvia",
    "Germany",
    "Czech Republic",
  ],
  geo: { lat: 53.9006, lng: 27.559 },
  foundingDate: "2015",
  slogan: {
    ru: "Комфортный трансфер из Минска в аэропорты Вильнюса и Варшавы",
    en: "Comfortable transfers from Minsk to Vilnius and Warsaw airports",
    pl: "Komfortowe transfery z Mińska na lotniska w Wilnie i Warszawie",
    fr: "Transferts confortables de Minsk vers les aéroports de Vilnius et Varsovie",
  },
  paymentAccepted: ["Cash", "Credit Card"],
  currenciesAccepted: "EUR, USD, BYN, PLN",
};

export const INDEXNOW_KEY = "c8f3d2a1b94e47f6ae7c1d0b5a8e3c92";
