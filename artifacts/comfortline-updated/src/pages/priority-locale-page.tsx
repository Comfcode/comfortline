...
export default function PriorityLocalePage() {
  ...
  return <div className="min-h-screen bg-background text-foreground"><header className="border-b border-border"><div className="container mx-auto px-4 py-5 flex justify-between"><a href={pl?"/pl":"/fr"} className="font-bold text-xl">ComfortLine</a><nav className="flex gap-3 text-sm">{(["ru","en","pl","fr"] as const).map(code=><a key={code} href={route[code]} hrefLang={code} className={code===lang?"text-primary":"text-muted-foreground"}>{code.toUpperCase()}</a>)}</nav></div></header><main><section className="relative overflow-hidden min-h-[560px] flex items-center"><img src={images[0]} alt={h1} className="absolute inset-0 w-full h-full object-cover"/><div className="absolute inset-0 bg-black/65"/><div className="container relative mx-auto px-4 py-20 max-w-5xl text-white"><p className="text-primary uppercase tracking-widest text-xs mb-4">{pl?"Licencjonowany przewoźnik · od 2015 roku":"Transporteur agréé · depuis 2015"}</p><h1 className="text-4xl md:text-6xl font-bold max-w-4xl">{h1}</h1><p className="text-lg text-white/80 mt-6 max-w-3xl">{description}</p><a href="tel:+375447620649" className="inline-block mt-8 bg-primary text-primary-foreground px-7 py-4 rounded-xl font-bold">{pl?"Zarezerwuj transfer":"Réserver un transfert"}</a></div></section><section className="container mx-auto px-4 py-12 max-w-5xl"><h2 className="text-3xl font-bold mb-6">{pl?"Trasa i komfort podróży":"Itinéraire et confort du voyage"}</h2><div className="grid md:grid-cols-3 gap-4">{images.map((src,index)=><img key={src} src={src} alt={`${name} — ${index+1}`} loading={index?"lazy":"eager"} className="w-full h-56 object-cover rounded-2xl border border-border"/>)}</div></section><section className="container mx-auto px-4 py-12 max-w-5xl grid md:grid-cols-3 gap-6">{cards.map(([a,b])=><article key={a} className="border border-border rounded-2xl p-6"><h2 className="font-bold text-xl">{a}</h2><p className="text-muted-foreground mt-2">{b}</p></article>)}</section><section className="container mx-auto px-4 py-12 max-w-5xl"><h2 className="text-3xl font-bold">{pl?"Popularne trasy z Mińska":"Itinéraires populaires depuis Minsk"}</h2><div className="grid md:grid-cols-2 gap-3 mt-6">{related.map(item=><a key={item.key} href={item[lang]} className="border border-border rounded-xl p-4 hover:border-primary">{pl?item.plName:item.frName}</a>)}</div></section></main><footer className="border-t border-border mt-12"><div className="container mx-auto px-4 py-8 text-sm text-muted-foreground">© 2026 ComfortLine · +375 (44) 762-06-49 · info@comfortline.by</div></footer></div>;
}
